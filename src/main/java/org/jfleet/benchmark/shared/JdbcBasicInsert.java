...
public class JdbcBasicInsert {

    private static final String INSERT = "INSERT INTO bike_trip (tripduration, starttime, stoptime, start_station_id, start_station_name, "
    ...
    public JdbcBasicInsert() {
    }

    public void insertAllOneByOne(Connection conn, Stream<TripEntity> stream) throws JFleetException, SQLException {
        try (PreparedStatement pstmt = conn.prepareStatement(INSERT)) {
            Iterator<TripEntity> iterator = stream.iterator();
            while (iterator.hasNext()) {
                TripEntity entity = iterator.next();
                setEntity(pstmt, entity);
                pstmt.executeUpdate();
            }
        }
    }
    
    public void insertAllBatch(Connection conn, int batchSize, Stream<TripEntity> stream) throws JFleetException, SQLException {
        try (PreparedStatement pstmt = conn.prepareStatement(INSERT)) {
            Iterator<TripEntity> iterator = stream.iterator();
            int cont = 0;
            while (iterator.hasNext()) {
                setEntity(pstmt, iterator.next());
                pstmt.addBatch();
                cont++;
                if (cont % batchSize == 0) {
                    pstmt.executeBatch();
    ...
    private void setEntity(PreparedStatement pstmt, TripEntity entity) throws SQLException {
