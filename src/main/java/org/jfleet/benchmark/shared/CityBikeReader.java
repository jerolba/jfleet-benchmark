...
public class CityBikeReader<T> {
                ...
                .forEach(f -> {
                    try (ZipInputStream zis = new ZipInputStream(new FileInputStream(f))) {
                        ...
                        while (ze != null) {
                            ...
                            Stream<T> trips = reader.lines().skip(1).map(l -> csvParser.apply(l)).map(counter).map(globalCounter);
    ...
    /**
     * Read each CSV file containing and apply to consumer an stream for each file
     * CSV found Each CSV file is parsed in different stream.
     *
     * @param consumer
     */
    public long forEachCsv(Consumer<Stream<T>> consumer) {
        StreamCounter<T> globalCounter = new StreamCounter<>(0);
                ...
                .forEach(f -> {
                    try (StreamCounter<T> counter = new StreamCounter<>()) {
                        Stream<T> trips = Files.lines(Paths.get(f.getAbsolutePath())).skip(1)
                                .map(l -> csvParser.apply(l)).map(counter).map(globalCounter);
                        consumer.accept(trips);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
